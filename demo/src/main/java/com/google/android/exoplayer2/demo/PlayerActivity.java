...
//      return;
//    }
        Uri source = Uri.parse(getIntent().getStringExtra("url"));
        if (Util.maybeRequestReadExternalStoragePermission(this, source)) {
            // The player will be reinitialized if the permission is granted.
            return;
        }
        MediaSource[] mediaSources = new MediaSource[1];
//    for (int i = 0; i < uris.length; i++) {
        mediaSources[0] = buildMediaSource(source, null);
//    }
...
//  }
    ...
    private DataSource.Factory buildDataSourceFactory(boolean useBandwidthMeter) {
        return ((DemoApplication) getApplication())
                .buildDataSourceFactory(useBandwidthMeter ? BANDWIDTH_METER : null, getIntent().getStringExtra("token"), getIntent().getStringExtra("agent"));
    }
